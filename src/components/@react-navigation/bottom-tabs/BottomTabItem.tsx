...
import { useTheme, type Route } from '@react-navigation/native';

import Color from '@components/@react-navigation/bottom-tabs/Color';
import TabBarIcon from '@components/@react-navigation/bottom-tabs/TabBarIcon';
...
/**
 * `@react-navigation/bottom-tabs`의 `BottomTabItem` 컴포넌트
 *
 * 기존: `activeOpacity`, `inactiveOpacity`에 따라 `icon`을 2개 렌더링
 *
 * 변경: `activeOpacity`, `inactiveOpacity` 및 `opacity` 관련 스타일을 제거하고 `icon`을 1개 렌더링
 *
 * 테스트 코드 작성 X
 * @jinhok96 25.05.26
 */
export default function BottomTabItem({
  ...
  const inactiveTintColor =
    customInactiveTintColor === undefined
      ? variant === 'material'
        ? // @ts-expect-error - rgb in definitely exists
